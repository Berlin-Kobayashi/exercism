...
func TestOnlyCallOnceOnMultipleDepChanges(t *testing.T) {
	...
	if changed4 < 1 {
		...
		t.Fatalf("callback function was called too often %d", changed4)
	}
}
...
func TestNoCallOnDepChangesResultingInNoChange(t *testing.T) {
	...
	inp.SetValue(5)
	if timesCalled != 0 {
		t.Fatalf("callback function called even though computed value didn't change %d %d", timesCalled, output.Value())
	}
}
